...
class EspetaculoTuna:
    ...
    def get_agenda(filtros: dict = None, apenas_futuros: bool = False):
        ...
        with db.cursor() as cursor:
            cursor.execute(
                f"""SELECT e.*, 'ISPGAYA' AS local
                   FROM espetaculos_tuna e
                   WHERE {where}
    ...
    def get_all_paginated(filtros: dict = None, pagina: int = 1, por_pagina: int = 9):
        ...
        with db.cursor() as cursor:
            cursor.execute(f"SELECT COUNT(*) as total FROM espetaculos_tuna WHERE {where}", params)
    ...
    def toggle_destaque(esp_id: int) -> bool:
        ...
        try:
            with db.cursor() as cursor:
                cursor.execute(
                    "UPDATE espetaculos_tuna SET destaque = CASE WHEN destaque = 1 THEN 0 ELSE 1 END WHERE id = ?",
                    (esp_id,)
                )
            db.commit()
            return True
    ...
    def criar(dados: dict) -> bool:
        ...
        try:
            with db.cursor() as cursor:
                cursor.execute(
                    """INSERT INTO espetaculos_tuna (titulo, descricao, tipo, data_evento, hora, entrada, preco, imagem, duracao, limite_filas, data_publicacao, destaque)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (
                        sanitize(dados.get('titulo', '')),
                        sanitize(dados.get('descricao', '')),
                        dados.get('tipo', 'atuacao'),
            ...
            db.rollback()
            print(f"Erro ao criar evento da tuna: {e}")
            return False
    ...
    def editar(esp_id: int, dados: dict) -> bool:
        ...
        try:
            with db.cursor() as cursor:
                cursor.execute(
                    """UPDATE espetaculos_tuna SET titulo=?, descricao=?, tipo=?,
            ...
            db.rollback()
            print(f"Erro ao editar evento da tuna: {e}")
            return False
    ...
    def eliminar(esp_id: int) -> bool:
        ...
        try:
            with db.cursor() as cursor:
                cursor.execute("UPDATE espetaculos_tuna SET ativo = 0 WHERE id = ?", (esp_id,))
            db.commit()
            return True
        except Exception:
            db.rollback()
    ...
    def get_galeria():
        ...
        with db.cursor() as cursor:
            cursor.execute(
                """SELECT g.* FROM galeria_tuna g
                   INNER JOIN espetaculos_tuna e ON g.espetaculo_id = e.id
                   WHERE g.ativo = 1
    ...
    def get_fotos_by_espetaculo(esp_id: int):
        ...
        with db.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM galeria_tuna WHERE espetaculo_id = ? AND ativo = 1 ORDER BY ordem ASC, criado_em DESC",
                (esp_id,)
            )
            return cursor.fetchall()
    ...
    def adicionar_foto_galeria(esp_id: int, titulo: str, imagem: str) -> bool:
        ...
        try:
            with db.cursor() as cursor:
                cursor.execute(
                    """INSERT INTO galeria_tuna (espetaculo_id, titulo, imagem, ativo)
                       VALUES (?, ?, ?, 1)""",
            ...
            db.rollback()
            print(f"Erro ao adicionar foto à galeria da tuna: {e}")
            return False
    ...
    def eliminar_foto_galeria(foto_id: int) -> bool:
        ...
        try:
            with db.cursor() as cursor:
                cursor.execute("UPDATE galeria_tuna SET ativo = 0 WHERE id = ?", (foto_id,))
            db.commit()
            return True
        except Exception:
            db.rollback()
...
class MembroTuna:
    ...
    def criar(dados: dict) -> bool:
        ...
        try:
            with db.cursor() as cursor:
                cursor.execute(
                    """INSERT INTO membros_tuna (nome, email)
                       VALUES (?, ?)""",
                    (
            ...
            db.rollback()
            print(f"Erro ao registar membro na tuna: {e}")
            return False
    ...
    def verificar_duplicado(email: str) -> bool:
        ...
        with db.cursor() as cursor:
            cursor.execute(
                "SELECT id FROM membros_tuna WHERE email = ?",
                (sanitize(email),)
            )
    ...
    def get_all():
        db = get_db()
        with db.cursor() as cursor:
            cursor.execute("SELECT * FROM membros_tuna ORDER BY criado_em DESC")
            return cursor.fetchall()
    ...
    def get_by_id(membro_id: int):
        db = get_db()
        with db.cursor() as cursor:
            cursor.execute("SELECT * FROM membros_tuna WHERE id = ?", (membro_id,))
            return cursor.fetchone()
    ...
    def atualizar_estado(membro_id: int, estado: str) -> bool:
        ...
        try:
            with db.cursor() as cursor:
                cursor.execute(
                    "UPDATE membros_tuna SET estado = ? WHERE id = ?",
                    (estado, membro_id)
                )
            db.commit()
            return True
            ...
            db.rollback()
            print(f"Erro ao atualizar estado do membro da tuna: {e}")
            return False
